...
# Task that can be used with multiprocessing to test
# SafeFileCreation by creating a number of files in a temp dir.
def _create_N_temp_files(tmp_dir, num_files_to_create):
    for i in range(num_files_to_create):
        filename = f'{tmp_dir}/tmp_file_{i}'
        with SafeFileCreation(filename) as sfc:
            pid = os.getpid()
            with open(sfc.tmpPath, 'w') as out:
                print(f"file number {i} created by process {pid}", file=out)
